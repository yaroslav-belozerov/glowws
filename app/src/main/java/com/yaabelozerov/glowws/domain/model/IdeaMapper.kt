...
class IdeaMapper {
    fun toDomainModel(mp: Map<Group, List<Idea>>, onClick: (Long) -> Unit): Map<GroupDomainModel, List<IdeaDomainModel>> {
        val out: MutableMap<GroupDomainModel, List<IdeaDomainModel>> = mutableMapOf()

        for ((key, value) in mp) {
            out[GroupDomainModel(key.groupId, key.name)] = value.map { IdeaDomainModel(it.ideaId, { onClick(it.ideaId) }, it.content) }
        }
        return out
    }
}