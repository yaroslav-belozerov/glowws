...
interface ModelDao {
    ...
    @Query("SELECT * FROM model WHERE isChosen = 1")
    suspend fun getLastActiveModel(): Model?

    @Query("UPDATE model SET isChosen = 0")
    suspend fun clearChosen()

    @Upsert
    suspend fun upsertModel(model: Model): Long
