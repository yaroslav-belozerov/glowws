...
class InferenceRepositoryImpl(
    ...
    private val _source = MutableStateFlow<Triple<Model?, InferenceManagerState, Long>>(
        Triple(
            null, InferenceManagerState.IDLE, -1L
        )
    )
    ...
    override suspend fun loadModel(model: Model, callback: suspend () -> Unit) {
        ...
        when (model.type) {
            ModelVariant.ONDEVICE -> localInferenceManager.activateModel(model.path!!) {
                _source.update {
                    it.copy(
                        model, InferenceManagerState.ACTIVE
                    )
                }
            }
    ...
    override suspend fun removeModel(model: Model, stateAfter: InferenceManagerState) {
        ...
        when (model.type) {
            ModelVariant.ONDEVICE -> localInferenceManager.removeModel(model.path!!) {
                _source.update {
                    if (it.first != model) it.copy(
                        second = stateAfter
