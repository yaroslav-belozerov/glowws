...
enum class SettingsCategories(val resId: Int, val icon: ImageVector) {
  DISPLAY(R.string.s_cat_display, Icons.Default.Star),
  USER(R.string.s_cat_user, Icons.Default.Person)
}
...
@JsonClass(generateAdapter = true)
data class SettingsList(@Json(name = "list") val list: List<SettingsModel>? = null)
