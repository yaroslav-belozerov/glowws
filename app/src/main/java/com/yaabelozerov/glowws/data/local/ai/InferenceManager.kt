...
class InferenceManager @Inject constructor(private val app: Context) {
  ...
  private val _callback: MutableStateFlow<Pair<(String) -> Unit, () -> Unit>> = MutableStateFlow(
    Pair(first = { st -> println("empty callback: $st") }, second = { println("empty callback: onEnd") })
  )
  ...
  suspend fun importModel(fileName: String, chan: ByteReadChannel, contentLength: Long?, callback: suspend (String) -> Unit = {}) {
    error.reset()
    withContext(Dispatchers.IO) {
      try {
        val dir = File(app.filesDir, "Models")
        ...
        try {
          ...
          while (!chan.isClosedForRead) {
            val read = chan.readAvailable(buffer)
            if (read > 0) {
              outStream.write(buffer, 0, read)
              bytesRead += read
              println("FILE PROGRESS: ${bytesRead / 1024 / 1024} / ${contentLength?.div(1024)?.div(1024)}")
            }
          }
  ...
  private fun setCallback(onUpdate: (String) -> Unit, onEnd: () -> Unit) = _callback.update { Pair(onUpdate, onEnd) }
